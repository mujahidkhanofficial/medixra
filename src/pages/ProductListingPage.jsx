...
const ProductListingPage = () => {
  ...
  useEffect(() => {
    loadProducts();
    if (user) {
      // Fix: Map the response to just listingIds
      const userWishlist = wishlistAPI.getByUserId(user.id) || [];
      setWishlist(userWishlist.map(item => item.listingId));
    }
  }, [filters, user]);
  ...
  const handleWishlist = (productId) => {
    ...
    if (wishlist.includes(productId)) {
      wishlistAPI.removeItem(user.id, productId);
      setWishlist(prev => prev.filter(id => id !== productId));
      toast({ title: 'Removed from wishlist' });
    } else {
      wishlistAPI.addItem(user.id, productId);
